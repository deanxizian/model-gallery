...
当前接入 10 款 Apple 设备及阅星瞳 X3。只有已完成的模型进入展厅，暂缓建模的产品不会生成空白卡片。每个 Apple 产品目录的 `product.json` 保留产品资料与已确认的拥有信息，`model.json` 控制页面展示；不把官方型号候选、容量和内存选项混作实机配置。待确认项目见 [待补充设备信息](device-records.md)。
