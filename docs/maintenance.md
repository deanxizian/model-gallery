...
这里保留添加模型、填写资料和维护网站时需要的详细说明。以下命令均在项目根目录执行。
...
X3 本体使用白色 Owned 工程，屏幕贴图保留用户提供的天气资讯界面，背标和触点材质按实物背面照片更新，GLB、封面与 P22 装配中的设备外观同步。参考照片留在本地，未作为贴图上传；机身与触点尺寸、位置保留。原阅读页模型和旧缩略图链接继续可用。转接头工程从生成当前 GLB 的 P21 装配场景提取，保留原有几何与材质。底座当前提供完整 P22 装配工程，充电头安装位置和触针动画同步下移 0.70 mm；网页预览由同版送印 STL 生成。背面外观更新不修改底座 STL 和 STEP。所有下载均直接复制对应源文件，旧版资源保留。
